...
- [OpenAI-Compatible Endpoint](guides/openaicompatible.md)
- [OpenRouter](guides/openrouter.md)
- [Poe](guides/poe.md)
- [Perplexity](guides/perplexity.md)
- [Vercel AI Gateway](guides/vercel.md)
- [xAI](guides/xai.md)
...
$env:OpenAICompatibleEndpoint="http://127.0.0.1:8000/v1" # for self-hosted OpenAI-compatible servers
$env:OpenAICompatibleKey="your-optional-endpoint-key"
```
...
Currently, tool calling is supported for the OpenAI, OpenAI-compatible, Vercel, xAI, Anthropic, and Google providers. Support for other providers will be added in future updates.

### OpenAI-compatible endpoints, including IREN-hosted deployments

Some infrastructure providers expose GPU capacity rather than a first-party
shared inference API. In those cases, deploy an OpenAI-compatible server such
as vLLM or TGI and point PSAISuite at it:

```powershell
$env:OpenAICompatibleEndpoint = 'http://127.0.0.1:8000/v1'
$env:OpenAICompatibleKey = 'optional-key-if-your-endpoint-requires-one'

Invoke-ChatCompletion `
    -Messages 'Summarize the deployment path.' `
    -Model 'openaicompatible:meta-llama/Llama-3.1-8B-Instruct'
```

See [guides/openaicompatible.md](guides/openaicompatible.md) for the reusable
path used for IREN-hosted OpenAI-compatible inference.

Before a PowerShell tool is invoked, PSAISuite normalizes optional arguments
generated by the model. Empty values, disabled switches, and values outside a
command's declared validation range are omitted so commands such as `Get-Date`
can run reliably even when a model supplies placeholder arguments.
...
Tools can be command names or the tool schemas accepted by
`Invoke-ChatCompletion`. The default is no tools, model `openai:gpt-5.6-luna`,
